...
`pairing-admin` 是一个 Hermes gateway 插件，用于把用户准入流程交给管理员审批。未授权用户可以触发准入申请，管理员可以在聊天里完成批准、忽略、拒绝、移除、拉黑和成员备注管理。

这个插件独立于 Hermes 官方平台插件。它注册 `pre_gateway_dispatch` hook，在 Hermes 常规授权流程之前处理符合条件的未授权用户。
...
公开文档中不要写真实 user ID、OpenID、API key、token、服务器地址或部署密钥。请使用 `qqbot:YOUR_OPENID`、`qqbot:USER_ID` 这类占位符。
